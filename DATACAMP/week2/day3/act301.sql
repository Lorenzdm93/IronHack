...
#2 List districts together with total amount borrowed and average loan amount.
select * from district;
select * from account;
select * from loan;

select acc.account_id, sum(amount) as totamount, avg(amount) as avgloan
from bank.account acc
join bank.district d
on d.A1 = acc.district_id
join bank.loan l
on acc.account_id = l.account_id
